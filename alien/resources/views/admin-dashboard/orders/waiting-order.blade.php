...
@section('heading','Waiting Order Information')
@section('content')
...
<div class="col-md-12">
  <div class="col-md-12 panel">
    <div class="col-md-12 panel-heading">
      <div class="col-md-3">
        <h4>Waiting Order List <a href="" style="margin-left: 10px;"> <i class="fa fa-refresh" aria-hidden="true"></i></a></h4>
      </div>
    ...
    <div class="col-md-12 panel-body padding-bottom30">
      <div class="col-md-12">
        <span class="bold text-primary">Total number of Waiting Order </span> ({{$qWaitingOrder->total()}})
        <table class="table-striped" style="width:100%;">
          ...
          @php($sl=1)
          @if(!empty($qWaitingOrder))
          @foreach($qWaitingOrder as $sWaitingOrder)
          <tr>
            <td style=" text-align: center;">{{$sWaitingOrder->id}}</td>
            <td style=" text-align: center;">{{$sWaitingOrder->name}}</td>
            <td style=" text-align: justify;">{{$sWaitingOrder->phone}}</td>
            <td style=" text-align: center;">{{$sWaitingOrder->created_at}}</td>
            <td style=" text-align: justify;">{{$sWaitingOrder->address}}</td>
            <td style=" text-align: center;">
              @if($sWaitingOrder->is_deliver=='Y')
              <button class="btn btn-sm btn-success">Delivered</button>
              @elseif($sWaitingOrder->is_deliver=='N')
              <button class="btn btn-sm btn-primary">Pending</button>
              <!-- <i class="fa fa-close text-danger"></i> -->
              @endif
            </td>
            <td style=" text-align: center;">
              <a href="{{url('invoice-details-by-order-id', $sWaitingOrder->id)}}" class="btn btn-sm btn-info"><i class="fa fa-eye"></i></a>
            </td>
          </tr>
